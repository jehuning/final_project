...
/* 페이지리스트 가로정렬 */ 
	...
	*/	
	...
	$(function() {
		...
		$("#SearchBtn").on('click', function() {		
			...
			$.ajax({
				...
				data :				
				{"condition":condition, "start":start, "finish":finish},
				success : function(result) {

					var html = jQuery('<div>').html(result);
					var contents = html.find("div#consultList").html();
	...
	<script>
	/*
	function consultPop(c) {
		var i = $(c).children().find('input').val();
		...
		var url = "/cpop"
		var name = "상세 출결현황";
		var option = "width = 520, height = 800, top = 100, left = 200, location = no, scrollbars = yes";
	...
	*/
	...
	$(function() {
		...
		$("#SaveBtn").on('click', function() {		
			 ...
			 var content = $('#scheduleSaveResult').val();
...
<!-- SIDEBAR -->
	...
	<div id="SelectCustomerNumb">
		...
		<!-- 조회 종류 선택 -->
		<select id="CustomerPrintCount" name="CustomerPrintCount" onchange="searchCondition(this)" >
			<option value=""selected disabled hidden>조회 방식 선택</option>
			<option value="ALL">전체 상담 일자별 조회</option>
			<option value="RG">우수고객 상담 일자별 조회</option>
			<option value="MK">마케팅 상담 일자별 조회</option>
			<option value="">마케팅 행사별 상담 조회</option>
		</select>


		<!--마케팅행사종류선택-->
		
			<select id="CustomerGrade" name="CustomerGrade" onchange="mkCondition(this)" disabled>
		...
		<!--Btn-->
		<a class='btn SubmitBtn' id="SaveBtn">상담 내용 등록</a>
